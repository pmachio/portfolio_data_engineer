...
docker exec -it $(docker-compose -f entorno/docker-compose.yml ps -q spark-worker) /spark/bin/spark-submit --packages org.apache.spark:spark-sql-kafka-0-10_2.12:3.1.2 --class org.uam.masterbigdata.drivers.StreamDriver --deploy-mode client --master spark://spark-master:7077  --supervise /opt/spark-apps/spark_project.jar
