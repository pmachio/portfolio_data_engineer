...
import org.pmachio.spark.metrics.influxdb.{InfluxDbReporter, InfluxDbSender}

import java.util


/**
 * based on GraphiteSink.scala from the Spark codebase and InfluxDBSink (https://github.com/palantir/spark-influx-sink/blob/master/spark-influx-sink/src/main/scala/org/apache/spark/metrics/sink/InfluxDbSink.scala)
 *
 */
...
/*TODO cambiar por bucket y organizacion
...
//  val database = propertyToOption(INFLUX_KEY_DATABASE).get
  ...
  LOG.warn(s"pollPeriod:$pollPeriod, pollUnit:$pollUnit")

  MetricsSystem.checkMinimalPollingPeriod(pollUnit, pollPeriod)
  //Reporter
  //TODO pendiente impleter
  ...
  val token = "98ccxq061TqUyWXJZ_OJC3KkzqQ-JKYXojx4xB8R22FCTT_Pp6nySUXf2BJNdDZBpX8q24S4G_FhDUFzsBnCvg==";
  val bucket = "metrics";
  val org = "metrics";
  val url = "http://localhost:8086"
  val sender = new InfluxDbSender(url, token, bucket, org)
  ...
  override def start() {
    ...
    fields.put("test",0.asInstanceOf[Object])
    sender.writePointMeasument("START", System.currentTimeMillis(), fields)
